...
D5 deliverable. Every number below points at a specific file or ADR in this repo —
...
• Ran a genuinely new 12-question held-out evaluation against a live RAG+SQL agent
  ...
  would have invalidated the whole point of holding it out. (docs/report.md D5, ADR-019)
...
• Built an internal Q&A API that routes between SQL (business data) and document
  ...
  violations across 25 questions. (docs/report.md D2)
...
`docs/report.md`'s D5 section and ADR-019 show happened, with the server log lines
to back it up.
